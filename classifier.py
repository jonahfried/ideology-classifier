...
def main():
    ...
    device, n_gpu = get_device_and_n_gpu(args.no_cuda, args.local_rank)
    ...
    if not args.skip_train: 
        train_examples = processor.get_train_examples(args.data_dir, args.num_train_examples, args.seed) # TODO merge with get_train_dataloader() ?
...
def train_model(model, optimizer, train_dataloader, num_train_epochs, gradient_accumulation_steps, device, n_gpu):
    ...
    # logger.info("   *****  STEP TRACKER *****  ")
    # logger.info(str(step_tracker))
    return step_tracker
...
def get_train_dataloader(train_examples, tokenizer, label_list, train_batch_size, max_seq_length, num_train_optimization_steps, local_rank):
    """ Convert training examples to a DataLoader  """
    train_features = convert_examples_to_features(train_examples, label_list, max_seq_length, tokenizer)
    log_training(len(train_examples), train_batch_size, num_train_optimization_steps)
...
def get_eval_dataloader(processor, data_dir, num_test_examples, eval_batch_size, max_seq_length, tokenizer, seed):
    """ parses test examples and prepares them into a DataLoader """

    eval_examples = processor.get_test_examples(data_dir, num_test_examples, seed)
    eval_features = convert_examples_to_features(eval_examples, processor.get_labels(), max_seq_length, tokenizer)

    log_evaluating(len(eval_examples), eval_batch_size)
...
class LeftRightProcessor:
    ...
    def get_examples(self, data_dir, num_examples, file, seed):
        ...
        data = data.sample(n=num_examples, random_state=seed)
    ...
    def get_train_examples(self, data_dir, num_train_examples, seed):
        return self.get_examples(data_dir, num_train_examples, "train.csv", seed)

    def get_test_examples(self, data_dir, num_test_examples, seed):
        return self.get_examples(data_dir, num_test_examples, "test.csv", seed)
...
def get_device_and_n_gpu(no_cuda, local_rank):
